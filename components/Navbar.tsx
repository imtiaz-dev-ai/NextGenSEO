...
const Navbar: React.FC<NavbarProps> = ({ currentRoute, setRoute }) => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[60] bg-[#020617] lg:bg-[#020617] border-b border-white/5 h-20 sm:h-24 flex items-center px-3 sm:px-6 md:px-12 justify-between" role="navigation" aria-label="Main navigation">
      <a
        href="/"
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden" onClick={() => setMenuOpen(false)}>
          ...
          <div className="pt-2 border-t border-white/5">
            ...
            <NavLink route={AppRoute.BUY_BACKLINKS} label="Marketplace" className={`block font-bold uppercase px-4 py-3 text-sm rounded-xl transition-colors ${currentRoute === AppRoute.BUY_BACKLINKS || currentRoute === AppRoute.MARKETPLACE ? 'text-purple-400 bg-purple-500/10' : 'text-slate-300 hover:text-white hover:bg-white/5'}`} />
          </div>
